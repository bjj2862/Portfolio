...
double ***d3tensor2(long nrl, long nrh, long ncl, long nch, long ndl, long ndh)
/* allocate a double 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
	long i,j,nrow=nrh-nrl+1,ncol=nch-ncl+1,ndep=ndh-ndl+1;
	double ***t;
	...
	/* allocate rows and set pointers to them */
	t[nrl][ncl]=(double *) malloc((size_t)((nrow*(ncol*(ndep+1)/2)+NR_END)*sizeof(double)));
	...
	for(j=ncl+1;j<=nch;j++) 
	{
		t[nrl][j]=t[nrl][j-1]+j-ncl;
	}
	for(i=nrl+1;i<=nrh;i++) 
	{
		t[i]=t[i-1]+ncol;
		t[i][ncl]=t[i-1][ncl]+(ncol)*(ndep+1)/2;
		for(j=ncl+1;j<=nch;j++) 
		{
			t[i][j]=t[i][j-1]+j-ncl;
		}
